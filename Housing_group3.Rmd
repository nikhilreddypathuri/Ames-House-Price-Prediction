...
author: 
  "<center>Sriudaya Damuluri, Dilip Kumar Molugu, Navya Shivaji Mote, Nikhil Reddy Pathuri, Luyao Shen, Xin Zhang</center>"
...
```
Of the 2909 houses with NA's for PoolQC only 3 recorded a PoolArea greater than 0. Since we cannot simply impute this based on other values, we decided to fill None for these values.
...
Only one house who had NA's in their garage columns had an area greater than 0. We can fill this house in manually and set the rest of the houses NA's to 0. 
...
From the correlation we can see the 10 features with the strongest effect on SalePrice.   
...
The houses with 'Partial' for SaleCondition have both the highest quality and highest sale price well above the other qualities. Instead of mapping each quality to its own numeric value we can one hot encode for houses with *Partial*
...
plsFit <- plsr(SalePrice~.,data = xt_train, ncomp = 20)
plsFit_pred<-predict(plsFit, x_test, ncomp = 20)
plsFit_pred_1 <- as.double(exp(plsFit_pred) - 1)
write.csv(plsFit_pred_1, file = "plsFit_pred.csv")
summary(plsFit)
...
nnetModel = train(x=x_train, y=y_train, method="nnet", linout=TRUE, trace=FALSE, MaxNWts=5 * (ncol(train1)+1) + 5 + 1, maxit=100, tuneGrid = nnGrid)
...
svmRModel = train(x=x_train, y=y_train, method="svmRadial", tuneLength=20, trControl = modelcontrol, preProc= pprocess)
svmRModel
plot(svmRModel)
...
marsModel = train(x=x_train, y=y_train, method="earth", tuneGrid=marsGrid, trControl = modelcontrol, preProc = pprocess)
